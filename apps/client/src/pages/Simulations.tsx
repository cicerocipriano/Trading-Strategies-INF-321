...
function SimulationRow({ simulation }: SimulationRowProps) {
    const {
        simulationName,
        strategyName,
        assetSymbol,
    ...
    return (
        <article className="ts-glass-surface ts-glass-hover-lift rounded-xl px-5 py-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="text-right min-w-[120px] space-y-1">
                <p
                    className={`flex items-center justify-end gap-1 text-sm md:text-base font-semibold ${isPositive ? 'text-emerald-400' : 'text-red-400'
                        }`}
                >
                    {isPositive ? (
